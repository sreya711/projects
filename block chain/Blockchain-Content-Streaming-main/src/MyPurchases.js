...
const MyPurchases = ({ contract, account }) => {
  ...
  const loadMyContents = async () => {
    ...
    const usersPurchases = await contract.getUsersPurchases();
    console.log(usersPurchases);
    // console.log(usersPurchases.length);

    let contents = []
    for (let i = 0; i < usersPurchases.length; i++) {
      console.log("for entered");
      console.log(usersPurchases[i]);
      const id = ethers.utils.formatUnits(usersPurchases[i], 18) * (10 ** 18)
      console.log(id);

      const content = await contract.contents(ethers.utils.formatUnits(usersPurchases[i], 18) * (10 ** 18))
      // const content = await contract.contents(1)
      console.log("for entered");
